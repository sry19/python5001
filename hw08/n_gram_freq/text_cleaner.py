...
class TextCleaner:
    ...
    def separate_sentences(self, file_content):
        ...
        self.word_lst.extend(file_content_lst)
    ...
    def count_bi_word(self):
        for idx in range(len(self.word_lst)-1):
            self.nf_bi.add_item(self.word_lst[idx]+'_'+self.word_lst[idx+1])
        print("Top", N, "bigrams:")
        for pair in self.nf_bi.top_n_freqs(N):
            print("  ", pair)

    def count_tri_word(self):
        for idx in range(len(self.word_lst)-2):
            self.nf_tri.add_item(self.word_lst[idx] + '_'
                                 + self.word_lst[idx+1]
                                 + '_' + self.word_lst[idx+2])
