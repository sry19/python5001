...
def sierpinksi(bottomLeft, bottomRight, top, recursionDepth):
    ...
    if recursionDepth == 0:
        ...
        left = bottomLeft.midPoint(top)
        right = bottomRight.midPoint(top)
        oldtop = bottomRight.midPoint(bottomLeft)
        triangle(left.getX(), left.getY(),
             right.getX(), right.getY(),
             oldtop.getX(), oldtop.getY())
        sierpinksi(top.midPoint(bottomLeft),top.midPoint(bottomRight),top, recursionDepth - 1)
        sierpinksi(bottomLeft,bottomLeft.midPoint(bottomRight),bottomLeft.midPoint(top), recursionDepth - 1)
        sierpinksi(bottomRight.midPoint(bottomLeft),bottomRight,top.midPoint(bottomRight), recursionDepth - 1)
