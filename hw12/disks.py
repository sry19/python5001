...
class Disks:
    ...
    def computer_ai(self):
        ...
        for i in range(self.row):
            for j in range(self.column):
                if self.disks_lst[i][j] == INITIAL_VAL and \
                     self.is_valid('white', i, j):
                    tmp_set = self.choose_best_pos(i, j)
                    if len(tmp_set) > len(flip_set):
                        flip_set = tmp_set
                        pos = (i, j)
        return pos

    def choose_best_pos(self, row, col):
        '''flips the disks'''
        color = WHITE
